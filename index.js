...
let validInputs = function () {
  ...
  //to dont  register if there is similar data in locale storage
  ...
  if (
    ...
    five.classList.contains("sucess") && radioBtns[0].checked === true || radioBtns[1].checked === true){
    if (arr.some((n) => n.email === email)) {

      alert("there is an account with this mail ,plz write another mail or maybe you have account lets login in")
    }
...
function checkMail(email) {
  ...
  if (email !== "" && email.match(emailRegex)) {
    ...
    failFun(mail, "Enter Valid Email like name.word@com");
  }
}
...
function checkPass(password) {
  if (password !== "" && password.length >= "9") {
    ...
    failFun(pass,"not valid, must be 9 char or greater");
  }
}
